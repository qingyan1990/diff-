...
class AstSimplifier
  ...
  def find_end_line(end_idx)
    @line_starts.each_with_index{|val,idx|
      if val >= end_idx
        return idx
      end
    }
      ...
      if obj.is_a?(Hash)
        ...
        obj.each do |k, v|
          if k == :location
            ...
            ret[:start_line] = v[0]
            #ret[:end_line] = v[1]
            ret[:end_line] = find_end_line(end_idx)
            whole_start = start_idx
            whole_end = end_idx
            start_line = v[0]
            #end_line = v[1]
            end_line = find_end_line(end_idx)
          else
  ...
  def convert(exp)
    if exp == nil
      ...
      case exp[0]
        ...
        when :params
          ret = {:type => :params, :fields => [:positional]}
          if exp[1]
        ...
        when :method_add_block
          ...
          call[:fields] << :args if not call[:fields].include? :args
          call[:args][:type] = :blockargument
          call[:args][:fields] = [:blockarg]
          call
        when :method_add_arg
          call = convert(exp[1])
        ...
        when :begin
          bodystmt = exp[1]
          {
              ...
              :fields => [:body, :rescue, :else, :ensure]
          }
        when :rescue
          ret = {:type => :rescue}
          ret[:fields] = [] if not ret[:fields]
          if exp[1]
...
simplifier = AstSimplifier.new(ARGV.first)
