...
def diff(file1, file2, move=False, parent=False):
    cleanup()
    if file1.endswith(".rb"):
        LANGUAGE = "ruby"
        s1 =  Popen("ruby ast.rb " + file1, shell=True, stdout=PIPE).stdout.read()
        s2 =  Popen("ruby ast.rb " + file2, shell=True, stdout=PIPE).stdout.read()
        h1 = loads(s1)
        h2 = loads(s2)
        node1 = Parser().convert(h1)
        node2 = Parser().convert(h2)
        changes, cost = diff_node(node1,node2,move,parent)
        print changes
